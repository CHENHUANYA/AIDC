...
function renderLookupSuggestion(lookup) {
  ...
  const fallback = !reaction && !remedy && !explanation ? compactText(text, 520) : '';

  return `<div class="suggestion-card">
    <div class="suggestion-head">
      <div>
        <div class="suggestion-kicker">Alarm ${app.esc(lookup.code || '')}</div>
        <div class="suggestion-title">${app.esc(lookup.title || 'Manual match')}</div>
      </div>
      <div class="suggestion-page">P.${app.esc(lookup.page || '-')}</div>
    </div>
    ${renderSuggestionSection('先做這些', compactText(remedy || fallback), 'primary')}
    ${renderSuggestionSection('機台反應', compactText(reaction), 'warn')}
    ${renderSuggestionSection('原因說明', compactText(explanation))}
    ${renderSuggestionSection('恢復方式', compactText(program))}
  </div>`;
}
  ...
  return `<div class="suggestion-card">
    <div class="suggestion-head">
      <div>
        ...
        <div class="suggestion-title">${app.esc(first)}</div>
      </div>
    </div>
    ${renderSuggestionSection('建議處理', compactText(body, 900), 'primary')}
  </div>`;
}
  ...
  try {
    ...
    if (/^\d{2,6}$/.test(alarmCode)) {
      ...
      if (!app.$('issueDescription').value.trim()) {
        setOperatorSuggestion(`查無 Alarm ${alarmCode}。請確認手冊與警報代碼，或補上問題描述後再查詢 RAG 建議。`, true);
        return;
      }
    }
    ...
    if (!suggestion) {
      throw new Error('LLM 回傳空白內容');
    }
    app.setState('operatorLastSuggestion', suggestion);
    setOperatorSuggestion(renderChatSuggestion(suggestion), false, true);
    app.$('operatorSuggestionFeedback').style.display = 'flex';
  } catch (error) {
    setOperatorSuggestion(app.formatError(error, '查詢初步建議失敗'), true);
  }
}
...
async function sendOperatorSuggestionFeedback(type) {
  ...
  try {
    ...
    setIssueResult(type === 'good' ? '已記錄：初步建議有幫助。' : '已記錄：初步建議沒幫助。');
  } catch (error) {
...
function getOperatorIssue(issueId) {
  const app = operatorApp();
  return app?.getState('operatorIssuesById')?.[issueId] || null;
}

function getOperatorWorkOrder(workOrderId) {
  const app = operatorApp();
  return app?.getState('operatorWorkOrdersById')?.[workOrderId] || null;
}

function setOperatorIssue(issue) {
  const app = operatorApp();
  if (!app || !issue?.issue_id) {
    return;
  }
...
function bindOperatorIssueCardEvents() {
  ...
  if (!app || !list || list.dataset.bound === 'true') {
    return;
  }
...
function operatorIssueGroup(issue) {
  ...
  if (issue.work_order_id || ['assigned', 'in_progress'].includes(issue.status)) {
    return 'maintenance';
  }
  return 'intake';
}
...
function renderOperatorIssueCard(issue) {
  ...
  const editedBadge = issue.updated_at && issue.updated_at !== issue.created_at
    ? `<span class="wo-badge">已更新 ${app.esc(formatOperatorTime(issue.updated_at))}</span>`
    : '';
  ...
  return `<div class="wo-card operator-issue-card" data-issue-id="${operatorAttr(issue.issue_id)}" tabindex="0" role="button">
    ...
    <div class="wo-meta">
      <span class="wo-badge">設備 ${app.esc(issue.machine_id || 'N/A')}</span>
...
async function createOperatorIssue(createWorkOrder) {
  ...
  try {
    ...
    if (data.status !== 'ok') {
      throw new Error(data.message || '建立問題失敗');
    }

    app.$('issueAlarmCode').value = '';
    app.$('issueDescription').value = '';
    const suffix = data.work_order ? `，已通知維修並建立工單 #${data.work_order.id}` : '';
    setIssueResult(`已建立問題 ${data.issue.issue_id}${suffix}`);
    setOperatorIssue(data.issue);
    if (data.work_order) {
      setOperatorWorkOrder(data.work_order);
    }
    loadOperatorIssues();
  } catch (error) {
...
function renderOperatorNotes(issue) {
  ...
  if (!notes.length) {
    return '<div class="wo-empty">尚無補充紀錄</div>';
  }
...
function renderOperatorIssueHistory(issue) {
  ...
  if (!history.length) {
    return '<div class="wo-empty">No audit events yet.</div>';
  }
...
function renderOperatorLinkedWorkOrder(issue) {
  ...
  const events = Array.isArray(order.work_order_history) ? order.work_order_history.slice().reverse() : [];
  return `<div class="operator-detail-block">
    <div class="operator-detail-label">Linked work order</div>
    ...
    ${events.length ? `<div class="audit-list compact-audit">${events.map((event) => `<div class="audit-event">
      <div class="audit-dot"></div>
      <div class="audit-body">
        <div class="audit-title">${app.esc(event.action || 'updated')}</div>
        <div class="audit-meta">${app.esc(event.user_id || 'system')} · ${app.esc(formatOperatorTime(event.created_at))}</div>
      </div>
    </div>`).join('')}</div>` : ''}
  </div>`;
...
function renderOperatorIssueFields(issue, isEditing) {
  ...
  if (!isEditing) {
    ...
    ${issue.rag_suggestion ? `<div class="operator-detail-block">
      <div class="operator-detail-label">送出時的初步建議</div>
...
function renderOperatorIssueActions(issue, isEditing) {
  ...
  return [
    ...
    issue.status === 'completed' ? `<button class="wo-btn alt" onclick="reopenOperatorIssue(${app.toJsArg(issue.issue_id)})">重新回報異常</button>` : '',
  ].filter(Boolean).join('');
}
...
function openOperatorIssueModal(issueId, mode = 'view') {
  ...
  app.$('operatorModalSub').textContent = `狀態 ${issueStatusLabel(issue.status)} · ${issue.work_order_id ? `維修工單 #${issue.work_order_id}` : '尚未通知維修'}`;
  detail.innerHTML = `${renderOperatorIssueFields(issue, isEditing)}
    <div class="operator-detail-block">
      <div class="operator-detail-label">補充紀錄</div>
      <div id="operatorNotesList">${renderOperatorNotes(issue)}</div>
      ${canAddOperatorNote(issue) && !isEditing ? '<textarea class="wo-text operator-note-input" id="operatorNoteInput" placeholder="補充最新觀察，例如異常是否仍持續、機台畫面新增訊息"></textarea>' : ''}
    </div>
    <div class="operator-detail-block">
...
function closeOperatorIssueModal() {
  const app = operatorApp();
  app?.$('operatorIssueModal')?.classList.remove('show');
}
...
async function patchOperatorIssue(issueId, payload, successMessage) {
  ...
  try {
    ...
    if (data.status !== 'ok') {
      throw new Error(data.message || '更新問題失敗');
    }
    setOperatorIssue(data.issue);
    if (data.work_order) {
      setOperatorWorkOrder(data.work_order);
...
async function saveOperatorIssueEdit(issueId) {
  const app = operatorApp();
  const issue = getOperatorIssue(issueId);
  if (!app || !issue || !canEditOperatorCore(issue)) {
    return;
  }
...
async function addOperatorIssueNote(issueId) {
  ...
  if (!app || !note) {
    setIssueResult('請先輸入補充內容。', true);
    return;
  }
  await patchOperatorIssue(issueId, { operator_note: note }, `已新增 ${issueId} 的補充紀錄`);
}

async function verifyOperatorIssue(issueId) {
  await patchOperatorIssue(issueId, { status: 'verified', operator_note: 'Operator confirmed resolved.' }, `已確認 ${issueId} 已解決`);
}
...
async function reopenOperatorIssue(issueId) {
  ...
  const note = app?.$('operatorNoteInput')?.value.trim();
  if (!note) {
    setIssueResult('請先填寫重開原因或仍然異常的現象。', true);
...
async function escalateOperatorIssue(issueId) {
  ...
  try {
    ...
    if (data.status !== 'ok') {
      throw new Error(data.message || '通知維修失敗');
    }
    setOperatorIssue(data.issue);
    if (data.work_order) {
      setOperatorWorkOrder(data.work_order);
    }
    const suffix = data.created ? `，已建立工單 #${data.work_order.id}` : `，已有工單 #${data.work_order_id}`;
...
function renderOperatorIssues(issues, stats) {
  ...
  if (!visible.length) {
    list.innerHTML = '<div class="wo-empty">目前沒有未處理問題</div>';
    return;
  }
  ...
  list.innerHTML = `<div class="operator-issue-groups">
    ${renderOperatorIssueGroup('未通知維修', grouped.intake)}
    ${renderOperatorIssueGroup('維修處理中', grouped.maintenance)}
    ${renderOperatorIssueGroup('待確認', grouped.confirm)}
  </div>`;
  return;

  list.innerHTML = visible.map((issue) => {
    const workOrderBadge = issue.work_order_id
      ? `<span class="wo-badge">維修工單 #${app.esc(issue.work_order_id)}</span>`
      : '<span class="wo-badge">尚未通知維修</span>';
    const editedBadge = issue.updated_at && issue.updated_at !== issue.created_at
      ? `<span class="wo-badge">已更新 ${app.esc(formatOperatorTime(issue.updated_at))}</span>`
      : '';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (app.currentUser?.()?.role !== 'operator') {
    app.$('issueLine').value = '';
  }
